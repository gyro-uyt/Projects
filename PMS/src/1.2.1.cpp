...
void PMS::cipherProfileData()
{
    ...
    Problem with this:
        You're reading line by line, so getline() strips the newline (\n) and you manually add it back.
        This works for displaying, but it's messy for XOR encryption, since newline chars get treated like normal data.
        Every time you decrypt, you're appending new \ns → leading to file corruption.
    */
    string content((istreambuf_iterator<char>(read)), istreambuf_iterator<char>());
    /*
    This is a one-liner that says:
        “Read the entire file byte-by-byte from beginning to end and dump it into a string.”
        istreambuf_iterator<char>(read) → starts reading at the beginning
        istreambuf_iterator<char>() → default-constructed = end of stream
        string( ... , ... ) → constructs a string from that whole range

    Result:
        You get the exact raw content of the file (no stripping \n, \r, or weird stuff)
    ...
    */
    ...
    // over-write the file, Write back in binary mode to avoid modifying line endings
    ofstream write(profile_name_path, ios::binary);
    // write << content;
    // This writes the content back as pure binary. It does not treat \n, \0, or anything else specially — just raw bytes.
